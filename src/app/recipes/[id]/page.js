...
export default function RecipeDetailPage() {
  ...
  const nutrition = {
    calories: recipe.CaloriesTotales || 0,
    protein: recipe.ProtéinesTotales || 0,
    carbs: recipe.GlucidesTotaux || 0,
    fat: recipe.LipidesTotaux || 0
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="p-6">
              ...
              <div className="flex flex-wrap items-center gap-4 mb-6">
                <div className="flex items-center">
                  ...
                  <span className="text-gray-600">{recipe.TempsDePréparation} min</span>
                </div>
                <div className="flex items-center">
                  <User size={18} className="text-gray-500 mr-2" />
                  <span className="text-gray-600">{recipe.NombreDePersonnes} pers.</span>
                </div>
                {recipe.TypeDePlat && (
                ...
                {recipe.IntolérancesAlimentaires && recipe.IntolérancesAlimentaires.map(allergen => (
                  <span 
                    key={allergen}
              ...
              <div className="border-b border-gray-200">
                <div className="flex -mb-px">
                  <button 
                    className={`py-3 px-6 font-medium ${activeTab === 'ingredients' ? 'text-green-600 border-b-2 border-green-600' : 'text-gray-500 hover:text-gray-700'}`}
                    onClick={() => setActiveTab('ingredients')}
                  >
                    Ingrédients
                  </button>
                  <button 
                    className={`py-3 px-6 font-medium ${activeTab === 'preparation' ? 'text-green-600 border-b-2 border-green-600' : 'text-gray-500 hover:text-gray-700'}`}
              ...
              <div className="py-6">
                {activeTab === 'ingredients' && (
                  <div>
                    <p className="text-gray-500 mb-4">Pour {recipe.NombreDePersonnes} personnes</p>
                    <ul className="space-y-2">
                      {recipe.Ingrédients && recipe.Ingrédients.map((ingredient, index) => (
                        <li key={index} className="p-3 bg-gray-50 rounded-lg flex justify-between">
                    ...
                    {recipe.IntolérancesAlimentaires && recipe.IntolérancesAlimentaires.length > 0 && (
                      <div className="mt-6 bg-yellow-50 border border-yellow-200 p-4 rounded-lg">
                        <div className="flex">
                          ...
                          <div>
                            <p className="font-medium text-yellow-800">Information allergie</p>
                            <p className="text-yellow-700 mt-1">
                              Cette recette contient des allergènes: {recipe.IntolérancesAlimentaires.join(', ')}.
                            </p>
                          </div>
                        </div>
                      </div>
        ...
        <div className="lg:col-span-4">
          <div className="bg-white rounded-lg shadow-sm p-6 mb-6 sticky top-4">
            <h2 className="text-xl font-semibold mb-4">Information nutritionnelle</h2>
