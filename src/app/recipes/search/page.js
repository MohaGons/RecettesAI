...
export default function SearchPage() {
  ...
  useEffect(() => {
    ...
    if (ingredientParam) setFilters(prev => ({ ...prev, ingredient: ingredientParam }));
  }, [searchParams]);
  ...
  const sortedRecipes = [...(recipes || [])].sort((a, b) => {
    if (sortBy === 'calories-asc') return (a.CaloriesTotales || 0) - (b.CaloriesTotales || 0);
    if (sortBy === 'calories-desc') return (b.CaloriesTotales || 0) - (a.CaloriesTotales || 0);
    if (sortBy === 'time-asc') return (a.TempsDePréparation || 0) - (b.TempsDePréparation || 0);
    if (sortBy === 'time-desc') return (b.TempsDePréparation || 0) - (a.TempsDePréparation || 0);
    ...
    <div className="container mx-auto px-4 py-8">
        ...
        <form onSubmit={handleSearch} className="flex flex-col md:flex-row gap-4">
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg flex items-center border ${
              showFilters ? 'bg-green-50 text-green-600 border-green-200' : 'bg-white border-gray-300'
            }`}
          >
        ...
        <div>
          {!isLoading && (
            <p className="text-gray-600">
              {sortedRecipes.length} résultat{sortedRecipes.length !== 1 ? 's' : ''} trouvé{sortedRecipes.length !== 1 ? 's' : ''}
              {query && ` pour "${query}"`}
            </p>
          )}
        </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12 bg-red-50 rounded-lg">
          <p className="text-red-600">Une erreur est survenue lors de la recherche.</p>
        </div>
      ) : sortedRecipes.length === 0 ? (
        <div className="text-center py-12 bg-gray-50 rounded-lg">
          <p className="text-gray-600 mb-2">Aucune recette ne correspond à votre recherche.</p>
          <p className="text-gray-500">Essayez d&apos;autres mots-clés ou de modifier vos filtres.</p>
        </div>
      ) : (
